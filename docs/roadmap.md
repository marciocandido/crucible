...
**Regra permanente (herdada da DEC-09, ver `docs/adr/ADR-001-escopo-v1.md`):** cada fase só começa quando a anterior provou valor em uso real. Funcionalidade fora da fase corrente nasce issue, nunca pauta.

Fonte original: `docs/fontes/crucible-fases.md`.
...
- Consolidação v1 (MOU) + Adendo 01 entram como `docs/` — são os ADRs fundadores (`docs/adr/ADR-001-escopo-v1.md` e `docs/adr/ADR-002-protocolo-execucao.md`)
...
**Estado atual:** documentação inicial concluída (este conjunto de documentos). O teste empírico do RES-10 e a implementação dos comandos `start`/`resume` ainda **não** foram executados — permanecem como bloqueio para fechar a Fase 0.
